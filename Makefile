CORE_FILES := src/csv.c src/point.c src/validation.c src/k_clustering.c
HEADER_FILES := src/point.h src/csv.h src/constants.h src/validation.h src/k_clustering.h

all: serial shared_cpu distributed_cpu shared_gpu distributed_gpu
serial: src/serial.c $(CORE_FILES) $(HEADER_FILES)
...
distributed_cpu: src/distributed_cpu.c src/distributed_cpu_k_clustering.c $(CORE_FILES) $(HEADER_FILES) src/mpi_util.c src/mpi_util.h
	mpicc -g -O0 -Wall -std=c99 -o output/distributed_cpu src/distributed_cpu.c src/distributed_cpu_k_clustering.c $(CORE_FILES) src/mpi_util.c
...
distributed_gpu: src/shared_gpu.c src/shared_gpu_k_clustering.c src/kernel.cu $(CORE_FILES) $(HEADER_FILES)
	...
	mpicc -g -Wall -std=c99 -c src/validation.c -o output/validation.o
	mpicc -g -Wall -std=c99 -c src/k_clustering.c -o output/k_clustering.o
